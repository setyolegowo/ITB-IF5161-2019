...
globals [
  ...
  blue-averagerent; to keep track of average rents of middle-class people in the city
...
to setup ;initial population and environment setup
  ...
  ask patches with [rent = 0 and abs pxcor <= initialcitylimit and abs pycor <= initialcitylimit]
    [
      set rent random-float 1 * (max-rent - min-rent) sprout 1
        [set breed households set old 0 set stay 0 set num-houses 1]
    ] ;patches with rent varying (normally distributed) between highest-rent and lowest-rent

  ask patches ;initially rent is the rent-payable. rent-payable changes and varies from rent during simulation.
    [set rent-payable rent]

  ask households ;initially the incomes are sent in accordance with the housing unit they are occupying. i.e. most prime land occupiers are rich, inadequate land occupiers are poor.
    [
      set income 3.3 * [rent] of patch-here

      ifelse random-float 1 < informalityindex
        [set informal? true]
        [set informal? false]
    ]
...
to settle-households
  ...
  ; move-to patch 0 0; start from center
    [move-to patch 0 0 find-house] ;and then roam around to search a house that is within income-constraints
end
;HOUSE SEARCH PROCESS
to find-house
  rt random-float 360 ;all directions
  fd random-float 1 ;one step at a time
  if (any? households-here with [color != [color] of myself]) or (rent-payable > 0.3 * income) or (not available?) ;;
...
to update-class
  if income > (mean [income] of households + 1.1 * standard-deviation [income] of households)
    [set color green set class-updated? true]

  if income < (mean [income] of households - 0.1 * standard-deviation [income] of households)
    [set color red set class-updated? true]

  if (income < (mean [income] of households + 1.1 * standard-deviation [income] of households))
        and (income > (mean [income] of households - 0.1 * standard-deviation [income] of households))
    [set color blue set class-updated? true]
end
...
to create-developer
  ask households-here
    [if not any? developers-here and not any? other households-here and Develop = true
      [
        hatch 1
